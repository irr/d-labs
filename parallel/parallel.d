...
int anOperation(string id, int duration)
{
    ...
    foreach (i; 0 .. (duration * 2)) {
        Thread.sleep(dur!"msecs"(500));  /* half a second */
        write(id.front);
        stdout.flush();
    }
...
void main()
{
    ...
    auto theTask2 = task((int value) {
            anOperation("inner task", value);
        }, 4);
    theTask2.executeInNewThread();

    auto theTask3 = task(&anOperation, "function's call", 2);
    theTask3.executeInNewThread();
